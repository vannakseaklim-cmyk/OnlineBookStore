...
export default function Index({ orders, auth, deliveries = []}) {
    ...
    const subtotal = selectedOrder?.items?.reduce((t, item) => {
        const price = item.book?.discounted_price
            ? parseFloat(item.book.discounted_price)
                        ...
                        {ordersList.map((order) => (
                            <div key={order.id} className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center">
                                <div>
                                    ...
                                    <p className="font-semibold text-lg">${Number(order.order_total + shippingFee).toFixed(2)}</p>
                ...
                <div className="p-0 bg-white rounded-xl overflow-hidden">
                    ...
                    <div className="p-6 bg-gray-50 border-t grid grid-cols-2 gap-4">
                        ...
                        <div className="text-right">
                            <p className="text-xs text-gray-400 uppercase">Total Paymented</p>
                            <span className="text-[#bda081] font-bold text-lg">
                        ...
                        {orders.links.map((link, index) => (
                            <button
                                ...
                                className={`px-3 py-1 rounded mx-1 ${
                                    link.active ? 'bg-[#bda081] text-white' : 'bg-gray-200 text-gray-700'
                                }`}
